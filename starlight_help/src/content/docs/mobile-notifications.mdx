...
Zulip Server 12.0+ and Zulip Cloud support end-to-end encryption for mobile push
notifications. All push notifications sent from an up-to-date version of
the server to an updated version of the app (30.0.271+ on Android, 30.0.272+ on iOS)
will be end-to-end encrypted. E2EE push notifications ensure that message content
and metadata (including the sender's name, and the channel and topic where the message was sent)
is not visible to intermediaries.
...
will not receive any push notifications. See [technical
...
<Steps>
  ...
  1. For organizations with more than 10 users, a user [who can manage plans
     and billing](/help/self-hosted-billing#configure-who-can-manage-plans-and-billing)
     needs to sign up for a [plan](https://zulip.com/plans/#self-hosted) for
     your organization.
</Steps>
