...
class UserPresenceAggregationTests(ZulipTestCase):
    ...
    def test_aggregated_presence_offline(self) -> None:
        ...
        with time_machine.travel(
            ...
            # After OFFLINE_THRESHOLD_SECS, the single-user presence endpoint
            # still returns the raw timestamps unchanged; offline status is
            # inferred by clients from timestamp age, not injected by the server.
            result = self.client_get(f"/json/users/{user.email}/presence")
        result_dict = self.assert_json_success(result)

        self.assertEqual(
            result_dict["presence"]["idle_timestamp"],
            datetime_to_timestamp(validate_time - timedelta(seconds=5)),
        )
