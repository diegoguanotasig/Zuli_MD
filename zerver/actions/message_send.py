...
def update_direct_message_group_stats(send_message_requests: Sequence[SendMessageRequest]) -> None:
    ...
    dm_group_ids = {req.direct_message_group.id for req in dm_group_send_requests}

    locked_dm_groups = (
        DirectMessageGroup.objects.filter(id__in=dm_group_ids)
        .select_for_update(no_key=True)
        .order_by("id")
    )

    direct_message_group_map = {dmg.id: dmg for dmg in locked_dm_groups}
