...
def do_update_embedded_bot_service(
    ...
    *,
    ...
    service_name_changed = service_name is not None and service_name != service.name
    if service_name_changed:
        assert service_name is not None
        service.name = service_name
        service.save(update_fields=["name"])

    existing_config = get_bot_configs([bot_profile.id]).get(bot_profile.id, {})
    config_changed = config_data is not None and dict(config_data) != existing_config
    if config_changed:
        assert config_data is not None
        BotConfigData.objects.filter(bot_profile=bot_profile).delete()
        do_update_bot_config_data(bot_profile, config_data)

    if not service_name_changed and not config_changed:
        return
    ...
    if config_changed:
        assert config_data is not None
        effective_config: dict[str, str] = dict(config_data)
    else:
        effective_config = existing_config
...
def do_update_incoming_bot_service(
    ...
    *,
    ...
    existing_config = get_bot_configs([bot_profile.id]).get(bot_profile.id, {})
    existing_integration_id = existing_config.get("integration_id")
    existing_pure_config = {k: v for k, v in existing_config.items() if k != "integration_id"}

    integration_id_changed = service_name is not None and service_name != existing_integration_id
    pure_config_changed = config_data is not None and dict(config_data) != existing_pure_config

    if not integration_id_changed and not pure_config_changed:
        return

    if integration_id_changed:
        assert service_name is not None
        set_bot_config(bot_profile, "integration_id", service_name)

    if pure_config_changed:
        assert config_data is not None
        BotConfigData.objects.filter(bot_profile=bot_profile).exclude(key="integration_id").delete()
        do_update_bot_config_data(bot_profile, config_data)
