...
import {dm_section_collapse_policy_values} from "./settings_config.ts";
...
function update_dm_section_height(dm_section: HTMLElement | undefined): void {
    if (!direct_messages_pinned || !dm_section) {
        document.documentElement.style.removeProperty("--dm-section-height");
        return;
    }

    const DM_SECTION_HEADER_HEIGHT = 27.43;
    const outer_height = $(dm_section).outerHeight(true);
    assert(outer_height !== undefined);
    document.documentElement.style.setProperty(
        "--dm-section-height",
        `${outer_height + DM_SECTION_HEADER_HEIGHT}px`,
    );
}
...
export function update_private_messages(): void {
    ...
    const dm_section = document.querySelector<HTMLElement>(
        "#left_sidebar_scroll_container #direct-messages-list-container",
    );

    if (dm_section) {
        const resize_observer = new ResizeObserver(() => {
            update_dm_section_height(dm_section);
        });
...
export function get_direct_messages_pinned(): boolean {
    return direct_messages_pinned;
}

export function set_direct_messages_pinned(pinned: boolean): void {
    direct_messages_pinned = pinned;
    $("#direct-messages-list-container").toggleClass("pinned", pinned);
...
export function initialize(): void {
    ...
    // Restore Pin status.
    if (user_settings.web_dm_collapse_policy === dm_section_collapse_policy_values.never.code) {
